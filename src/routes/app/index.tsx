...
function HomePage() {
  ...
  const tiles = [
    ...
    { to: "/app/grades", title: isStudent ? t.tabPoints : t.tabGrades, desc: isStudent ? t.tileGrades : t.tileGradesT, tone: "terra" as const, icon: Star },
  ...
  return (
    <div>
      ...
      <section className="mb-4 grid grid-cols-4 gap-3 max-[1080px]:grid-cols-2 max-[520px]:grid-cols-2">
        {isStudent ? (
          <>
            <Stat icon={<Star className="size-4" />} value={String(stats?.points ?? 0)} label={t.statPoints} />
...
function Stat({ icon, value, label }: { icon: ReactNode; value: string; label: string }) {
  return (
    <div className="flex items-center gap-3 rounded-[18px] border border-hairline bg-white/80 px-[18px] py-4">
